# INPUT one argument
  # file with ID in first column and location (month, year, plate) in the third
# OUTPUT one directory
  # new directory (cat_edits) with one fastq file for every ID, concatenated across all edit files
  

#!/bin/bash
mkdir cat_edits                                                             # make directory for concatenated fastq files
A=$(awk '{print $1}' $1 | uniq )                                            # define variable A as one ID per line, no duplicates
...
do
    B=$(echo $A | cut -d ' ' -f $i)                                         # define variable B as the ID within loop
